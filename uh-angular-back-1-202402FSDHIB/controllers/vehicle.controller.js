...
const vehicleController = {
    ...
    editVehicle:async(req, res)=>{
        ...
        const vehicle = await Vehicle.findById(id, {available}, {new: true});
        if(!vehicle){
            return res.status(404).json({ message: 'Vehiculo no encontrado'})
        }
// actualizo si el vehiculo esta o no disponible

vehicle.available = available || vehicle.available;

await vehicle.save();

res.status(200).json({message: 'Vehicle updated succesfully', vehicle});
